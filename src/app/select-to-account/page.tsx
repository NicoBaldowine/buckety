...
export default function SelectToAccountPage() {
  return (
    <ProtectedRoute>
      <SelectToAccountContent />
    </ProtectedRoute>
  )
}

function SelectToAccountContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { user } = useAuth()
  const [accounts, setAccounts] = useState<Account[]>([])
  const fromAccountId = searchParams?.get('fromId') || ''
  ...
  useEffect(() => {
    ...
    // Filter out the fromAccount if it's selected
    const filteredAccounts = fromAccountId 
      ? allAccounts.filter(account => account.id !== fromAccountId)
      : allAccounts
      ...
      <div className="max-w-[660px] mx-auto px-12 py-6 max-sm:px-4 max-sm:py-3">
        ...
        >
          {accounts.map((account) => {
            const isDisabled = !!account.targetAmount && account.currentAmount >= account.targetAmount
            return (
              <button
                key={account.id}
